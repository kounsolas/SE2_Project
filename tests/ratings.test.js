...
test('GET /ratings/{id} successful', async (t) => {
  const ratingId = "12345"; // Valid rating ID
  const restaurantName = "mamalouka";

  try {
    const response = await t.context.got.get(`ratings/${ratingId}?restaurant_name=${restaurantName}`, {
      responseType: 'json'
    });
    ...
    t.deepEqual(response.body, {
      ...
      rating: 4.5,
      restaurant_name: "mamalouka"
    });  // Assert the response matches the expected rating
  } catch (err) {
    console.error('Error: ', err.response.body);
    t.fail('Request failed with error: ', + err.message);
  }
...
test('GET /ratings/{id} fails when rating not found', async (t) => {
  ...
  const error = await t.throwsAsync(() => t.context.got.get(`ratings/${ratingId}?restaurant_name=${restaurantName}`, {
    responseType: 'json'
  }));
